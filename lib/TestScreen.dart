...
class _TestScreen extends State<TestScreen> {
  ...
  Future getImage() async {
    ...
    ui.Image renderedImage = await recorder
        .endRecording()
        .toImage(_uiimage.width,_uiimage.height);
  ...
  Future cropimage(List<Face> faces) async{
    ...
    final data = await _uiimage.toByteData(format: ui.ImageByteFormat.png,);

    final bytes = data?.buffer.asUint8List();

    file1 = await file1.writeAsBytes(bytes as List<int>, flush: true);

    img.Image? originalImage = img.decodeImage(File(file1.path).readAsBytesSync());
